...
const MileStoneDescriptionPreview = ({
  modal,
  isModalOpen,
  openModal,
  description,
  ...
  return (
    <>
      {isModalOpen && modal}
      <p
        className={`${styles.description.boxing} ${styles.description.text} ${styles.description.spacing}`}
      >
        {LimitedText(description, windowWidth > 768 ? "lg" : "sm")}
        {description.length >
          renderingTextNum[windowWidth > 768 ? "large" : "small"] && (
          <span className={`${styles.detail.text}`} onClick={openModal}>
            ...detail
          </span>
        )}
      </p>
...
export default MileStoneDescriptionPreview;
