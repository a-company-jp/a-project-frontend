...
const useImageUploder = () => {
    ...
    const upload = async (image: File): Promise<string | void> => {
        ...
        const ImageUploadRequest = protobuf.roots.default.lookupType("main.ImageUploadRequest");
        ...
        await axios.post(
            `${process.env.BACKEND_DOMAIN}/api/v1/image/upload`,
            buffer,
            {
                headers: {
        ...
        .then((response) => {
            const ImageUploadResponse = protobuf.roots.default.lookupType("main.ImageUploadResponse");
